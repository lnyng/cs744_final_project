...
object MyConsumerPerformance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val totalLatency = new AtomicLong(0)
    ...
    if (!config.useOldConsumer) {
      ...
      for (streamList <- topicMessageStreams.values)
        for (i <- 0 until streamList.length)
          threadList ::= new ConsumerPerfThread(i, "kafka-zk-consumer-" + i, streamList(i), config, totalMessagesRead, totalBytesRead, totalLatency, consumerTimeout)
    ...
    if (!config.showDetailedStats) {
      ...
      val avgLatency = (totalLatency.get * 1.0) / totalMessagesRead.get
      print("%s, %s, %.4f, %.4f, %d, %.4f, %.4f".format(
  ...
  private def printHeader(showDetailedStats: Boolean, useOldConsumer: Boolean): Unit = {
    val newFieldsInHeader = if (!useOldConsumer) ", rebalance.time.ms, fetch.time.ms, fetch.MB.sec, fetch.nMsg.sec" else ""
    if (!showDetailedStats) {
        println("start.time, end.time, data.consumed.in.MB, MB.sec, data.consumed.in.nMsg, nMsg.sec, avg.e2e.latency.ms" + newFieldsInHeader)
      } else {
  ...
  class ConsumerPerfThread(threadId: Int,
    ...
    override def run() {
      ...
      try {
        ...
        while (messagesRead < config.numMessages && iter.hasNext) {
          val messageAndMetadata = iter.next()
          messagesRead += 1
          bytesRead += messageAndMetadata.message.length
          val currentTimeMillis = System.currentTimeMillis
          val sendTimeMillis = ByteBuffer.wrap(messageAndMetadata.message).getLong
          latency += currentTimeMillis - sendTimeMillis
